    ...
    def predict(
        ...
        with torch.cuda.amp.autocast():
            output_path = sample_diffusion(text=prompt, bert=self.bert, ldm=self.encoder, model=self.model, clip_model=self.clip_model, custom_clip=clip, batch_size=batch_size,
                                        device=self.device, prefix=log_dir, timestep_respacing=timestep_respacing, ddpm=ddpm, guidance_scale=guidance_scale, shape=(width, height), wandb_run=None, images_per_row=images_per_row)
